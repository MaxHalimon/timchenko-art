...
8. **Живі курси валют:** `EXCHANGE_RATES_FROM_EUR` у `lib/currency.ts` — статичні заглушки, база — EUR (валюта, в якій художниця задає ціни). Перед запуском підключити реальний курс (напр. exchangerate-api.com) із періодичним оновленням/кешем.
...
13. **⚠️ Робота "14072024" (`prisma/paintings.json`, слаг `x14072024`) зображує впізнавану реальну публічну особу** (натяк на замах на Дональда Трампа 2024 року). Викладена в каталог як є, за прямим рішенням власника проєкту. Варто мати на увазі: продаж репродукцій/мерчу з портретом живого публічного діяча може мати юридичні нюанси в деяких юрисдикціях (право на власне зображення, право на публічність) — особливо якщо колись зʼявиться друк на інших носіях (футболки, постери тощо), а не лише продаж оригіналу. Перед виходом на ринки США/Німеччини — варто звіритись з юристом саме щодо цієї роботи.

## Нюанси, ще відкриті

1. **Валюта показу ≠ валюта списання (частково вирішено).** `PriceTag` показує ціну в обраній валюті (UAH/EUR/GBP/USD/JPY), і базова валюта БД тепер EUR — так само, як і фактичне списання через Stripe/NOWPayments (`currency: "eur"`). Тобто клієнт із карткою не в євро побачить на чекауті конвертацію від свого банку — це нормально й типово для мультивалютних сайтів, окремого рішення тут поки не потрібно.
...
4. **Назви й описи картин** (`Product.title`, `description`) зберігаються в `Json`-полі, окремим значенням на кожну мову (`{ uk, en, de, fr, ja }`) — переклад самого контенту галереї вже реалізований, окремих таблиць перекладів не потрібно.
